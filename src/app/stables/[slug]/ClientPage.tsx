...
export default function StableProfilePage() {
  ...
  const createFallbackStable = (stableSlug: string): Stable => ({
    ...
    services: [
      { id: '1', name: 'Lekcje jazdy', description: 'Indywidualne lekcje jazdy konnej', type: 'lesson', duration: 60, price: 100, skillLevel: 'beginner' },
      { id: '2', name: 'Treningi', description: 'Treningi dla zaawansowanych', type: 'training', duration: 90, price: 150, skillLevel: 'advanced' },
      { id: '3', name: 'Pensjonat', description: 'Pensjonat koni', type: 'boarding', duration: 30, price: 2000, skillLevel: null },
      { id: '4', name: 'Obozy jeździeckie', description: 'Obozy dla dzieci i młodzieży', type: 'other', duration: 480, price: 1500, skillLevel: 'beginner' },
    ],
    openingHours: {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 md:-mt-40 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8 order-2 lg:order-1">
            ...
            {/* Services */}
            <Card variant="elevated" className="border-none">
              <h2 className="text-2xl font-bold text-navy-900 mb-6 font-serif">Usługi</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {stable.services.map((service) => (
                  <div key={service.id} className="p-4 bg-navy-50 rounded-2xl border border-iceBlue/30">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-bold text-navy-900">{service.name}</h3>
                      <span className="text-lg font-bold text-oceanBlue">{service.price} zł</span>
                    </div>
                    {service.description && (
                      <p className="text-sm text-navy-600 mb-3">{service.description}</p>
                    )}
                    <div className="flex items-center gap-3 text-sm text-navy-700">
                      ...
                      {service.skillLevel && (
                        <span className="px-2 py-0.5 bg-oceanBlue/10 text-oceanBlue rounded-full text-xs capitalize">
                          {service.skillLevel === 'beginner' ? 'Początkujący' : service.skillLevel === 'intermediate' ? 'Średniozaawansowany' : 'Zaawansowany'}
                        </span>
                      )}
                    </div>
                  </div>
